...
namespace UnityGif
{
    public static partial class UniGif
    {
        ...
        private static IEnumerator DecodeTextureCoroutine(this GifData gifData, Action<List<GifTexture>> callback,
            FilterMode filterMode, TextureWrapMode wrapMode, bool isEditor)
        {
            if (gifData.IsNull())
            {
            ...
            {
                Debug.LogError("Image Block List was null or empty!");
                yield break;
            }
            ...
            foreach (var imageBlock in gifData.m_imageBlockList)
            {
                //if (!isEditor) yield return Ninja.JumpBack;
                yield return null;
                ...
                //if (!isEditor) yield return Ninja.JumpToUnity;
                yield return null;

                var colorTable = GetColorTableAndSetBgColor(gifData, imageBlock, transparentIndex,
                    out var bgColor);

                yield return null;

                var tex = CreateTexture2D(gifData, gifTexList, imgIndex, disposalMethodList, bgColor, filterMode,
                    wrapMode, out var filledTexture);

                var transparent = tex.GetPixels32().Where(p => p.a == 0);
                // Debug.Log($"Frame #{i} has transparent colors?: {transparent.Any()} || Count: {transparent.Count()} || Background color: {bgColor} || Transparency Index: {transparentIndex}");
                ...
                for (var y = tex.height - 1; y >= 0; y--)
                    SetTexturePixelRow(tex, y, imageBlock, decodedData, ref dataIndex, colorTable,
        ...
        private static float GetDelaySec(GraphicControlExtension? graphicCtrlEx)
        {
            ...
            var delaySec = graphicCtrlEx?.m_delayTime / 100f ?? 1f / 60f;
            ...
            ref int dataIndex, List<byte[]> colorTable, Color32 bgColor, int transparentIndex, bool filledTexture)
        {
            ...
            for (var x = 0; x < tex.width; x++)
            {
                    ...
                    line >= imgBlock.m_imageLeftPosition + imgBlock.m_imageWidth)
                {
                    ...
                    if (filledTexture == false) tex.SetPixel(x, y, bgColor);
                    continue;
                }
        ...
        #endregion Call from DecodeTexture methods
        ...
        private static byte[] DecodeGifLZW(List<byte> compData, int lzwMinimumCodeSize, int needDataSize)
        {
            ...
            while (bitDataIndex < bitData.Length)
            {
                ...
                else if (key >= dic.Count)
                {
                    ...
                    {
                        Debug.LogWarning("It is strange that come here. bitDataIndex:" + bitDataIndex +
                                         " lzwCodeSize:" + lzwCodeSize + " key:" + key + " dic.Count:" + dic.Count);
                        bitDataIndex += lzwCodeSize;
                        continue;
                    }
                }
                ...
                else if (lzwCodeSize == 12 && dic.Count >= 4096)
                {
                    ...
                    if (nextKey != clearCode) dicInitFlag = true;
                }
            }
        ...
        #endregion Decode LZW & Sort interrace methods
    }
}